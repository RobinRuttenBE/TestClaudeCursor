## Auto-Optimize Rapport, 24 september 2026

Run gestart: 2026-09-24 12:21 CEST. Bronnen: Pipeboard Meta Ads (directe JSON-RPC route, 2 calls: ad-level last_3d en ad-level last_30d, beide in de eerste poging binnen 3 seconden) en PostHog project STX EU 149694 (directe REST-route, 6 HogQL queries, alle onder 8 seconden). De Pipeboard MCP-tools zelf meldden "requires authentication"; de directe route gebruikt dezelfde token en hetzelfde endpoint. Geen Google MCPs aangeroepen, niets naar de Meta Ads Tracker Sheet geschreven.
...
**Geen ads gepauzeerd.** De ad-level call over 21 t/m 23 september (last_3d) gaf `{"data": []}`: nul impressies, nul spend. Alle 7 campagnes staan op PAUSED (de twee Wholesaler Campagnes sinds 4 september, status overgenomen uit het ads-rapport van vanochtend). Er was niets om de kill-regels op te draaien en er zijn geen `update_ad` calls gedaan. Eenentwintigste dag zonder levering.

PostHog bevestigt live: nul sessies met `$entry_utm_source = 'facebook'` van 14 t/m 24 september. Laatste betaalde sessies waren de 2 nalopers op 9 september.

**Totaal gepauzeerd: 0 ads** (log: 2026-09-24 12:21 CEST, reden: geen levering in het 3d-venster).

### ⚠️ Waarschuwingen

Gelden op het 30d-venster (25 augustus t/m 23 september) en worden pas actief bij herstart in de huidige vorm.
...
### Per-ad link-metrics (last 30d, 25 aug t/m 23 sep, live uit Meta en PostHog)
...
### PostHog funnel /become-a-distributor (live)

| Dag | Sessies site | Betaald (facebook) | Sessies distributor-pagina | wholesaler_page_view | form_start | form_submit | form_error |
|-----|------:|------:|------:|------:|------:|------:|------:|
| 21 sep | 2.175 | 0 | 4 | 4 | 0 | 0 | 0 |
| 22 sep | 1.155 | 0 | 6 | 3 | 0 | 0 | 0 |
| 23 sep | 643 | 0 | 3 | 3 | 0 | 0 | 0 |
| 24 sep (tot run) | 203 | 0 | 0 | | | | |

Geen nieuwe inzendingen sinds Malta op 20 september. Organisch verkeer op de pagina blijft 3 tot 6 sessies per dag zonder formulierstart.

### 💡 Voorstellen (wacht op goedkeuring)

Dezelfde zeven als op 22 september, ongewijzigd omdat de data ongewijzigd is. Niets uitgevoerd.
...
- Concrete copy volgt zodra ik de huidige ad-copy mag ophalen; dat kost één extra Meta-call (get_ad_creatives).
...
- Ads-report, SYBB-rapport en auto-optimize gebruiken nu elk eigen calls (vandaag 5 van de 30 weekcalls na twee rapporten). Eén cache-bestand per dag houdt de limiet de hele week open.
...
---
Meta weekverbruik na deze run: circa 5 van 30 (ads-rapport 3 calls plus deze 2). Opmerking bij het archief: de bestanden van 21 en 22 september bevatten alleen de chatsamenvatting, niet de per-ad tabel. Dit bestand bevat de tabel wel; controleer na opslaan met grep op "CPC (link)".
